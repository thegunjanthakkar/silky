...
(function() {
  ...
  function updateCountDown(countDownItem) {
    let dateStr = countDownItem.getAttribute('data-count');
    ...
    if (/^\d{2}[\/\-]\d{2}[\/\-]\d{4}$/.test(dateStr)) {
        ...
        // If format is yyyy-mm-dd, convert to yyyy/mm/dd to ensure JS parses it in Local Time instead of UTC
   ...
   * Automatically syncs guest cart to database after login
   */
  ...
  // Initialize cart sync check when DOM is ready
  document.addEventListener('DOMContentLoaded', function() {
    // Skip cart sync on checkout page to prevent form submission interference
    if (window.skipCartSync !== true) {
      checkAndSyncCart();
    } else {
      console.log('Cart sync skipped on checkout page');
    }
  });
